...
const paint = {
  ...
  gray: (text) => (supportsColor ? `\x1b[90m${text}\x1b[0m` : text),
};
...
function createRenderer() {
  ...
  let mode = "none"; // 'none' | 'text' | 'reasoning'
  let buffer = "";
...
async function run(ctx, config) {
  ...
  const banner = (sessionId) =>
    ...
    paint.gray(`model: ${selectionRef.current.model} (${selectionRef.current.provider})\n`) +
    paint.gray(`session ${String(sessionId)}\n`) +
    paint.gray("Type /help for commands, /exit or Ctrl-D to quit.\n\n");

  stdout.write(
    banner(currentAgent.id) +
      (config.resumeSessionId.trim() !== "" ? paint.gray("(resumed)\n") : ""),
  );
